...
REFERNCE_DEGC = 0
CORRECTION_STR = f'corrected to {REFERNCE_DEGC}°C'
...
REF_YEAR = 2015
df['findpeaks_drift'] = (df.findpeaks_deviation -
                         df.findpeaks_deviation[REF_YEAR])
df['findpeaks_room_drift'] = (df.findpeaks_room_deviation -
                              df.findpeaks_room_deviation[REF_YEAR])
df['gain_drift'] = df.gain_deviation - df.gain_deviation[REF_YEAR]
df['gain_room_drift'] = (df.gain_room_deviation -
                         df.gain_room_deviation[REF_YEAR])

output_csv = BASE_NAME + '.csv'
logger.info('Saving: %s', output_csv)
...
logger.info('Saving: %s', summary_csv)
...
for (index, note), ax in zip(SUBSET_NOTES, axes):
    sns.distplot(df[index][REF_YEAR], ax=ax, color='0.5',
                 vertical=False, hist=True, rug=True, kde=False,
                 fit=stats.norm, fit_kws=dict(linestyle='--'))
    sns.distplot(df[index][REF_YEAR].drop(OUTLIERS), ax=ax, color='0.5',
                 vertical=False, hist=False, rug=False, kde=False,
                 fit=stats.norm)

    _annotate(
        ax, STATS_FMT % stats.norm.fit(df[index][REF_YEAR].drop(OUTLIERS)),
        'upper right')
...
for (index, _), ax in zip(SUBSET_NOTES, axes[:, 1]):
    sns.distplot(df[index].drop(REF_YEAR), ax=ax, vertical=True,
                 rug=True, kde=False, fit=stats.norm, color='0.5')
    _annotate(ax, STATS_FMT % stats.norm.fit(
        df[index].drop(REF_YEAR)), 'upper right')

    ax.set_ylabel('')
    ax.set_xticks([])

axes[1, 0].set_ylabel('Gain deviation from %s [%%]' % REF_YEAR)
